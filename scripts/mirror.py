#!/usr/bin/env python3
"""Mirror new arity releases from PyPI as commits and tags.

For every arity version on PyPI newer than the one pinned in
...
def current_version() -> str:
    match = re.search(r'"arity==([^"]+)"', PYPROJECT.read_text())
    assert match, "no arity pin in pyproject.toml"
    return match.group(1)


def pypi_versions() -> list[str]:
    with urllib.request.urlopen("https://pypi.org/pypi/arity/json") as response:
...
def set_version(version: str) -> None:
    ...
    pyproject = re.sub(r'"arity==[^"]+"', f'"arity=={version}"', pyproject)
